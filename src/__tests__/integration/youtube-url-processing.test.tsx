...
describe("YouTube URL Processing Integration Tests", () => {
    ...
    testCases.forEach(({ name, url, expectedVideoId }) => {
      test(`should process ${name} successfully through API`, async () => {
        ...
        expect(data.message).toContain(
          "YouTube video processing is currently under development",
        );
      });
      ...
      test(`should process ${name} successfully through UI`, async () => {
        ...
        const mockResponse = {
          ...
          message:
            "YouTube video processing is currently under development. Please check back later for full functionality.",
        };
        ...
        expect(
          screen.getByText("Platform detected: YouTube"),
        ).toBeInTheDocument();
        ...
        await waitFor(() => {
          ...
          expect(
            screen.getByText(
              new RegExp(`Title:.*YouTube Video ${expectedVideoId}`),
            ),
          ).toBeInTheDocument();
        });
      });
  ...
  describe("Error Handling Integration", () => {
    test("should handle invalid non-YouTube URL through complete workflow", async () => {
      ...
      expect(
        screen.getByText("Platform detected: Unknown"),
      ).toBeInTheDocument();
  ...
  describe("Platform Detection and Validation Consistency", () => {
    test("should have consistent behavior between API and UI for all platforms", async () => {
      ...
      for (const { url, platform, shouldWork } of testUrls) {
        ...
        expect(
          screen.getByText(`Platform detected: ${platform}`),
        ).toBeInTheDocument();
